<div class="swiper-slide bg-white h-auto group/item overflow-hidden shadow-md hover:shadow-lg rounded-xl relative">
    <div class="h-[240px] w-full block bg-cover bg-no-repeat bg-center relative" style="background-image: url({{$item->getFirstMediaUrl() ?: asset('images/item_placeholder.jpg')}})" data-hover-slides="@foreach($item->getMedia()->take(5) as $image) {{$image->getUrl()}}@if(!$loop->last),@endif @endforeach">
    ...
    <div class="p-6 space-y-6">
        ...
        <div class="flex justify-between space-x-1 items-center relative">
            <a href="tel:{{$item->user?->phone ?: \App\Models\Setting::where('code', 'phone')->first()->value}}" class="flex items-center justify-center space-x-1 border border-slate-300 rounded-md py-2 w-full hover:bg-red-500 hover:border-red-500 hover:text-white">
                <x-icons.phone class="h-5 w-5"/><span>Позвонить</span>
            </a>
            <a href="mailto:{{$item->user?->email ?: \App\Models\Setting::where('code', 'email')->first()?->value}}" class="flex items-center justify-center space-x-1 border border-slate-300 rounded-md py-2 w-full hover:bg-red-500 hover:border-red-500 hover:text-white">
